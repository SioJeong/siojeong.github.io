...
컴퓨터 공학에서 프로그램의 **Semantics(의미론)** 은 컴퓨터가 그 프로그램을 실행할 때 어떻게 동작하는지를 의미한다.

시맨틱은 명령어를 표현하는 특정한 형태인 구문론(Syntax)와 대조된다.

예를들어 자바스크립트에서 **i += 1** 과 **i = i + 1** 명령은 구문론적으로 다른 표현이지만, 의미론적으로는 같은 표헌이다.
